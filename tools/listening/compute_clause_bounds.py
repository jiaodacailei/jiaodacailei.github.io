...
用法：
  python compute_clause_bounds.py <enriched_combined.json> <合并后的音频>
      ...
      [--min-rise 6] [--quiet-ceiling -33] [--margin -0.02]
...
4. `quiet_min` 离 `rough_t` 太远（超过0.15秒）也要跳过——`char_times` 有
   时候会把两个不同位置的字符插值成完全相同的时间戳（真实案例：l14会话
   第9句"え"和"本当"都被记成0.31），紧跟在逗号后面那个 token 的有效跨度
   因此可能被拉得异常宽，窗口内找到的"最安静点"这时候容易越过 rough_t
   对应的这个字/词，跑到句子里后面完全不相关的地方（这个例子里真正命中
   的是"本当に"内部的某个自然停顿，不是逗号本身）——`rise`/`quiet_ceiling`
   两个条件都满足也不能豁免这一条，安静点本身响度再低，离猜测位置太远
   就已经不是"这个逗号的停顿"了。

**任何一个条件不满足就跳过这个逗号，不勉强给出一个不可靠的边界**——比如
...
的6），但 `min_db` 全部落在-30.2~-37.7 之间，被 `-38` 挡在外面。**这一步
犯过一次真实的错**：当时只人工核实了其中一个案例（l14会话第3句），确认
它是真停顿，就直接下结论"这10处rise都很明显，肯定都是真停顿，是-38太
严格误伤了它们"，把 `--quiet-ceiling` 一次性放宽到 `-28`——**没有逐条
核实剩下9处，想当然地把"rise够高"当成了"一定是真停顿"的充分条件**。
用户按左边列表"分句是否正好落在逗号处"这个更直接的标准一挑，很快就
挑出了反例：l16会话第19句"山田さん，お幸せに。"，`rise≈19.7`、
`min_db≈-30~-32`，跟被误杀的那9处几乎同一个量级，实际却是"山田さん"
和"お幸せに"之间根本没有真实停顿（人工核对过响度剖面，这段是连续说话，
没有任何深谷）——`-28` 把这类"就是比周围话音略低一点、但仍然是正常说话
音量"的普通波动也当成了真停顿放行。**教训：`rise` 高只能说明"窗口里
确实出现过响亮内容"，不能反推"最安静的那个点就一定是真停顿"——这两件
事看似相关，实际是两个独立的判据，`quiet_ceiling` 存在的意义正是补上
"这个最安静点本身够不够安静"这后半个判断，不能因为前半个信号强就放松
后半个**。现在改成 `-33`（介于两次判断之间，覆盖 l14 第3句的 -35.8，
同时挡住 l16 第19句这类 -30~-32 的假阳性）——**这个值本身也不是终点，
只是这次两个已知反例之间的折中，以后遇到新的反例（不管是该找到没找到、
还是找错了地方）都要用"分句是否正好落在逗号处"这个标准核实，不能只看
rise/min_db数值本身觉得"看起来够高就该信"。
...
def find_clause_bounds_for_sentence(audio_path, s, args):
    ...
    for i, ch in enumerate(text):
        ...
        # 真实案例（l14会话第9句"いいえ，本当に気持ちだけですから。"）：char_times
        # 有时候会把两个不同位置的字符插值成完全相同的时间戳（这句里"え"和
        # "本当"都是0.31）——说明底层数据本身就没能分辨这两个位置，"本当"这个
        # token的有效跨度因此被拉得异常宽（0.31~0.71，紧跟着的"に"要到0.71才
        # 出现）。窗口内如果找到的安静点离 rough_t（逗号后一个字符本该在的位置）
        # 太远，很可能已经越过 rough_t 对应的这一个字/词、跑到句子里后面完全
        # 不相关的地方去了（这个例子里真正命中的是"本当に"内部的某个自然停顿，
        # 不是逗号本身）——`char_times` 本身文档过的误差范围是"0.1~0.3秒"，
        # 超出这个范围就不再信任，宁可跳过也不要给一个偏到别的词上的假边界。
        if abs(min_t - rough_t) > 0.15:
            skipped.append((i, rough_t, f"找到的安静点({min_t})离逗号预期位置太远(>0.15s)，"
                                         f"可能是句子里别处的停顿，不是这个逗号的，跳过"))
            continue
        ...
        # 过安全的搜索范围。
        #
        # 光靠固定 margin 还不够——真实案例（l13会话第2句"課長，お忙しいところ
        # 申し訳ありませんが，ちょっとよろしいですか。"）：min_t 只比 rough_t
        # （"ちょっと"自己的时间戳）晚0.03秒，margin=-0.02 抵消完还剩0.01秒，
        # 结果这个边界还是比"ちょっと"晚，前端按 token 时间戳分组时"ちょっと"
        # 还是被分进了逗号前面那一段，看着像没跟着逗号切。这里 rough_t 本身就是
        # "紧跟在逗号后面那个 token 自己的时间戳"，只要边界严格早于 rough_t，
        # 这个 token 就一定会被分到正确的一侧——不管 margin 算出来多大，都再夹一
        # 道"不晚于 rough_t 前 0.01 秒"，直接从根上保证这一点，不再依赖 margin
        # 这个固定值凑巧够不够大。
        new_bound = round(max(min(min_t + args.margin, hi, rough_t - 0.01), lo), 2)
...
def main():
    ...
    ap.add_argument("--quiet-ceiling", type=float, default=-33.0,
