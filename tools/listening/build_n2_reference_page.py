...
TTS_READING_OVERRIDES = {
    ...
    # 走到这条替换。
    "扇ぐ": "あおぐ",
    # "怪しむ"（あやしむ，感到奇怪/觉得可疑）同样是孤立词条标题没有上下文，
    # edge-tts猜错了读音——真实反馈"怪しむ（あやしむ）的音频发音不对"。
    # "怪しむ"这个动词只有这一种读音（不像"町"要看上下文才知道该读まち还是
    # ちょう），不需要限定上下文，全局替换即可；不会误伤"怪しまない"等其它
    # 活用形，因为那些字符串里不包含"怪しむ"这个精确子串。
    "怪しむ": "あやしむ",
    # N2语法页的语法点标题带半角括注（"语法结构本身的一部分"，见
    # listening-page.js的TITLE_TRAILING_FULLWIDTH_PAREN_RE注释——半角括号
    # 不当成读音提示去掉，原样喂给TTS），孤立成标题、没有例句那种自然语境
    # 兜底时，edge-tts容易读错——真实反馈"6. ～上で(は)的音频发音不对"。
    # "上で"单独出现会被猜成"上手で"（じょうずで），放进完整例句反而没事
    # （"外国語を勉強する上で..."读音正确，有前后文帮它消歧），只有孤立
    # 标题这种没有上下文的场景需要订正；"上で(は)"整体替换成"うえでは"，
    # 连括注里的"は"一起读出来，不是"うえで"读完就断掉。
    "上で(は)": "うえでは",
    # "あげく(に)"/"あまり(に)"这两个词本身读音没有歧义（不像"上で"会被
    # 猜成别的词），但孤立标题+半角括注这个组合会让edge-tts在读到括号处
    # 插入一个奇怪的顿挫/多余音（真实测试：单独合成"～あげく(に)"听起来
    # 是"あげくっに"，"～あまり(に)"是"あまりずに"，都不是干净的读音），
    # 干脆把括注展开、直接给一个连读的干净读音。
    "あげく(に)": "あげくに",
    "あまり(に)": "あまりに",
    # "10. ～(よ)うではないか/(よ)うじゃないか"——derive_reading()按"/"
    # 切出的第一段是"(よ)うではないか"，开头就是半角括注，edge-tts完全
    # 猜不出这是"よ"这个假名的可选前缀，读成了"用腕"之类不相干的词。
    # 展开成"ようではないか"（比"うではないか"更自然，"よ"更常读出来）。
    "(よ)うではないか": "ようではないか",
    # "18. ～が契機で/..."——derive_reading()取第一段"が契機で"喂给TTS，
    # "契機"（けいき，契机/时机）edge-tts猜成了同音词"ケーキ"（蛋糕），
    # 真实测试孤立测过"契機"/"が契機で"两种输入都会读成蛋糕。这个词只有
    # "けいき"一种读音，全局替换不会误伤（不会跟"型（けい）"之类别的词
    # 撞在一起，"契機"是精确两字组合）。
    "契機": "けいき",
    # "0083. 泡（あわ）"孤立成词条标题+例句里单独出现的"泡"，edge-tts猜成了
    # 音读"ほう"——真实反馈"0083. 泡（あわ）的音频不对哟"。"泡"这个字在这
