...
_TOKEN_READING_OVERRIDES_UNCONDITIONAL = {
    ...
    # 显示假名注音。
    "〆": "しめ",
    # "一周"（转一圈/环游一周）pykakasi 默认给いっしゅう（词典最常见的标准
    # 读音），但N2词汇01"0144.一周"这个词条书上两条例句的振り仮名都明确
    # 标的是いちしゅう（"世界一周せかいいちしゅう""万博会場を一周いちしゅう
    # する"）——真实反馈"一周的注音标错了"，用户直接贴出书上两条例句的原始
    # 振り仮名。这个词在站内所有内容模块里只有这一个词条用到，全局覆盖
    # 不会误伤别的场景；词条标题本身的读音括注也同步改成了いちしゅう
    # （n2_vocab_content.py），跟例句保持一致。
    "一周": "いちしゅう",
