...
_TOKEN_READING_OVERRIDES_BY_PREV = {
    ...
    ("王", "風"): "ふう",
    # "お金"（钱）里孤立的"金"字该读かね，pykakasi 默认给音读きん（"金メダル"
    # "純金"这类"金属/黄金"语境的读音）——"お"+"金"没有被合并成一个复合词
    # token，是分开的两个 token，"金"单独出现时几乎总是紧跟在"お"后面表示
    # "钱"这个训读义项。真实案例（N2语法01第1单元パートI問題3第21题
    # "お金をためたいと言っている一方で、無駄遣いをしている。"）：读成
    # 「おきん」，该是「おかね」。只在紧跟"お"后面时触发，不影响"金メダル"
    # "純金"等孤立"金"字本身合法读きん的场景。
    ("お", "金"): "かね",
}
...
_TOKEN_READING_OVERRIDES_UNCONDITIONAL = {
    ...
    # 保留せんじん这个读音的场景）。
    "千尋": "ちひろ",
    # "一晩中"（整晚，一整夜）该读ひとばんじゅう，pykakasi 按"一"的音读
    # いち＋"晩中"猜成いちばんちゅう——真实反馈"一晩中いちばんちゅう雨風が
    # 強かった。应该是hitoban"："一晩中"是固定复合词，"一"这里训读ひと
    # （一个晚上），不是数量词的音读いち，没有"いちばんちゅう"这个读法
    # 会用到的真实场景，可以放心无条件覆盖。
    "一晩中": "ひとばんじゅう",
    # "開ける"（打开，及物动词）该读あける，但 pykakasi 把词干+第一个送假名
    # "開け"当成一个整体 token 处理（不管后面接る/た/て/ます/ない哪种活用形，
    # 实测过这五种都一样，"開け"始终被合并成单独一个 token，不会拆成"開"+
    # "け"两个 token——原来按"orig=='開'紧跟next_char=='け'"设计的规则永远
    # 不会命中，就是因为想错了 token 边界），默认读音给的是ひらけ（"開く"
    # 这个词自己的训读"ひら"+送假名"け"，不是"開ける"的训读"あ"）。真实反馈
    # "雨戸を開ける。注音错误，应该是akeru"。"開け"作为独立词只有"開ける"
    # 这一种活用来源，没有撞见"開け"该读ひらけ的真实反例，放心无条件覆盖。
    "開け": "あけ",
    ...
    # 显示假名注音。
    "〆": "しめ",
    # "一昨日"（前天）pykakasi 把它拆成"一昨"+"日"两个token，"日"字默认猜成
    # ひ（"孤立の日"那条语境规则误伤，"一昨日"根本不是"Xの日"这种结构），
    # 拼出来变成"いっさくひ"——跟TTS_READING_OVERRIDES里同名词条是同一个
    # 真实反馈的两处症状（build_n2_reference_page.py那份只管TTS发音，这里
    # 管的是页面上显示的振り仮名注音，两条独立生效，缺一个都会导致这个词
    # 要么读错要么注音显示错）。"一昨日"在站内所有内容模块里只有这一个
    # 词条用到，全局覆盖安全。
    "一昨日": "いっさくじつ",
    # "一周"（环游一圈）pykakasi 自己的默认猜测本来就是正确的いっしゅう
    # （跟书上"0144.一周（いっしゅう）"词条原文一致），但 SudachiPy 交叉核对
    # 这一步把它错误覆盖成了いちしゅう——SudachiPy 把"一周"切成"一"（数詞，
    # 读イチ）+"周"（助数詞可能名詞，读シュウ）两个独立语素，当成"数字1+
    # 量词周"这种用法处理，不是当成"一周（环游/绕一圈）"这个固定复合词，
    # 拼出来变成いちしゅう。跟"北上"那条覆盖是同一类"两边不一致时优先信
    # SudachiPy"这条兜底策略本身在这个词上失灵的真实案例（见"北上"那条
    # 注释）。**真实踩过的坑**：一开始收到反馈"一周的注音标错了，应该是
    # せかいいちしゅう/いちしゅう"，没有先核对原文截图就直接按反馈文字把
    # 这里改成了"一周"→"いちしゅう"（还连带错误地把词条标题、TTS发音表都
    # 一起改了）——后来找到原文照片核实，书上明明白白写的是いっしゅう，
    # 反馈文字本身转述错了，之前那次"修复"反而把本来正确的注音改错。
    # 直接无条件覆盖回いっしゅう，绕开SudachiPy交叉核对；这个词在站内所有
    # 内容模块里只在这一个词条+对应的练习题干扰项里用到，没有"一周"该读
    # いちしゅう的真实场景。
    "一周": "いっしゅう",
    # "世界一周"（环游世界一圈）pykakasi 会把"世界"+"一周"整体合并成单独一个
    # token（不会拆成两个独立token，例句"世界一周"就是这个合并token本身），
    # 上面那条"一周"→いっしゅう的覆盖按精确orig匹配（`orig in dict`），匹配
    # 不到"世界一周"这个更长的合并token，需要单独再加一条——pykakasi给这个
    # 合并token的默认读音是せかいいちしゅう（"一周"部分同样错读成いちしゅう，
...
def _resolve_hira(orig, hira, prev_orig, next_char=None, prev2_orig=None):
    ...
    if orig == "歳" and prev_ends_with_digit:
        return "さい"
    # "年"跟"人"是同一类坑，甚至比"人"更极端：孤立成词的"年"（"その年"＝那一年、
    # "年を取る"＝上年纪、"年が改まる"＝辞旧迎新，独立名词表示"一年/年岁"）该读
    # とし，pykakasi给这类孤立token一律默认成ねん（数字+年份计数读法），不是
    # "大多数场景对、少数场景错"，是反过来"孤立年几乎全错，只有紧跟阿拉伯数字
    # 的计数场景（2020年＝にせんにじゅうねん）才对"——实测过"その年""年を取る"
    # "年が改まる"这几个日常最常见的独立名词用法，pykakasi 清一色给ねん，没有
    # 一个猜对的。"今年/来年/毎年/長年/何年/長年"这类固定复合词pykakasi会自己
    # 合并成一个token（分别读ことし/らいねん/まいとし/ながねん/なんねん），不会
    # 拆出孤立的"年"字，不受这条规则影响，跟"人"的"外国人/成人"同一个道理。
    # 跟"人"同理用prev_ends_with_digit判断：数字后面的"年"是计数读法ねん，其它
    # 场景（の/その/动词连体形等后面）几乎总是独立名词读法とし。真实案例
    # （N2词汇01"0067.改まる"例句"年が改まる。"）：读成ねんが，是错的，该是
    # としが——顺带发现比"0023.明くる"两条例句更普遍的同一类坑，不再局限于
    # "明くる"这一个特定搭配，改成跟"人"同样通用的条件。
    if orig == "年" and not prev_ends_with_digit:
        return "とし"
    ...
    if orig == "日" and prev_orig == "の":
        return "ひ"
    # "明くる日"（"下一个/第二个"，连体词"明くる"直接修饰紧跟的名词，
    # "明くる日"＝第二天）里的"日"该读ひ，跟上面"の日"→ひ是同一类"孤立单字
    # 默认读音在特定搭配下要变成独立名词读法"的坑——pykakasi给孤立的"日"默认
    # 读にち（计数/复合词读法），但这里单独表示"（某）一天"这个独立名词义项
    # 时该读ひ。"明くる"没有别的汉字可依附，pykakasi把它拆成"明く"+"る"两个
    # token（不是按单字拆的，"明く"合成一个token），用prev_orig=="る"、
    # prev2_orig=="明く"这个组合信号识别，不容易误伤别的"る"结尾token后面
    # 紧跟汉字的场景（真正的动词连体形修饰名词时，那个"る"通常会跟前面的
    # 汉字词干合成一个token，不会被拆成孤立单字"る"，"明くる"是没有别的
    # 汉字词干可用的特例）。真实案例（N2词汇01"0023.明くる"例句"明くる日"）：
    # 用户听出同一个词条另一条例句"明くる年"被读错（见上面"年"那条通用规则），
    # 顺带核对发现"明くる日"是同一类坑。
    if orig == "日" and prev_orig == "る" and prev2_orig == "明く":
        return "ひ"
    ...
    if orig == "交わ" and next_char in "すしさせ":
        return "かわ"
    # "扇"孤立成词紧跟送假名"ぐ"（"扇ぐ"＝扇风这个动词，词干"扇"训读あお）
    # 时，pykakasi默认给孤立的"扇"字读おうぎ（"扇子/扇"这个名词的读音，
    # 比如N2词汇02"0169.炒る/煎る"例句"たまごを炒る"旁边"0164.芋"惯用语
    # 例句"扇子で扇ぐ。"）——真实反馈"扇子で扇ぐ。注音错误，应该是
    # aogu"：furigana把"扇ぐ"的"扇"读成了おうぎ（名词读音），"ぐ"当成
    # 孤立送假名留白，正确应该是"扇[あお]ぐ"整体读あおぐ。只在紧跟送
    # 假名"ぐ"时触发，不影响"扇"单独作为名词（读おうぎ）的场景。
    if orig == "扇" and next_char == "ぐ":
        return "あお"
