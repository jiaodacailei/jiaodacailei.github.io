...
_TOKEN_READING_OVERRIDES_BY_PREV = {
    ...
    ("5", "日"): "いつか",
    # "19日"（じゅうくにち，规则读法）——pykakasi 自己的默认值本来就是正确的
    # にち，但 SudachiPy 交叉核对这一步把它错误覆盖成了か（SudachiPy 对"19"
    # 这个数字token的处理本身有bug，"日"给出的读音か实际是"20日"=はつか才
    # 该有的读音，张冠李戴到了"19"上）——跟"北上"→きたかみ是同一类"SudachiPy
    # 自己出错、不是pykakasi的坑"的真实案例（textbook-sjp-zg-l18，两封信的
    # 日期都是"1月19日"）。显式登记回にち，绕开交叉核对。
    ("19", "日"): "にち",
...
_TOKEN_READING_OVERRIDES_UNCONDITIONAL = {
    ...
    # 时的具体 bug，不是上下文歧义）。
    "組に": "くみに",
    # "北上"（公司名"北上貿易"的一部分，textbook-sjp-zg-l18）pykakasi 自己的
    # 默认猜测本来就是正确的きたかみ（地名衍生的公司名），但 SudachiPy 交叉
    # 核对这一步把它覆盖成了错误的ほくじょう（"北上する"=向北推进/北上，这个
    # 常见动词义项在 SudachiPy 词典里优先级更高）——这不是 pykakasi 的坑，是
    # "两边不一致时优先信SudachiPy"这条兜底策略本身在这个词上失灵的真实案例
    # （skill 文档早就提醒过这类"SudachiPy自己也会出错的场景"不会被自动接住）。
    # 直接无条件覆盖回きたかみ，绕开交叉核对——这一课"北上"只出现在公司名
    # 里，没有真的表示"向北"这个动词义项的场景。
    "北上": "きたかみ",
    # "〆"（信件结尾表示"缄/封"的记号，读しめ，textbook-sjp-zg-l18生词表）
    # 不是CJK统一表意文字区的汉字（属于"CJK Symbols and Punctuation"区块），
    # pykakasi/SudachiPy都不把它当汉字处理，读音管线整段没有走到、直接
    # 留空——tokens里完全没有kana字段（跟"没识别出来读错"不是同一类问题，
    # 是"根本没被识别成需要注音的字符"）。这份quiz数据（build_vocab_quiz_
    # data.py走的是另一套独立管线）已经正确给出しめ，可以确认这是唯一
    # 正确读音，不是语境歧义。真实案例：用户人工核对页面发现"〆"没有
    # 显示假名注音。
    "〆": "しめ",
}

# 有些 token 全是汉字、没有送假名可当 _split_kana_segments() 的切分锚点，
# 会被那个函数的退化分支整段合并成一个 ruby——但退化分支假设的是"熟字训"
# （比如"女将"→"おかみ"，两个汉字的读音本来就没法拆开，合并成一段是唯一
# 正确的排版），有的词其实是每个汉字各自独立的训读，只是凑巧没有送假名
# 能当锚点，正确排版应该按汉字逐字拆开注音，不能跟真熟字训一概而论。
# 这张表直接给出人工核实过的拆分结果，绕开退化分支。
_TOKEN_SPLIT_OVERRIDES = {
    # "水炊き"（一种鸡肉火锅菜名）pykakasi 分词时把送假名"き"错误地划进了
    # 下一个 token（"水炊きなど"被切成 orig="水炊"/hira="みずたき" +
    # orig="きなど"/hira="きなど"，正确的词边界应该是"水炊き"+"など"）——
    # "水炊"这个 token 自己全是汉字，_split_kana_segments() 的退化分支只能
    # 整段一起注音，一开始只是把 hira 订正成"みずた"（3拍，留给紧跟着的
    # 字面"き"字符自己承担最后一拍），排版上显示成"水炊[みずた]"整体一个
    # ruby——但"水炊"不是熟字训，"水"读みず、"炊"读た是两个可以独立标注
    # 的训读（"炊く"读たく，连体形前的词干た跟"洗濯"→"洗濯"分开标是同一类
    # 排版规范），该拆成"水[みず]"+"炊[た]"两段而不是合并成一段。真实案例
    # （textbook-sjp-zg-l15，用户反馈"水炊应该分成两个kana"）。
    "水炊": [("水", "みず"), ("炊", "た")],
    # "MTS社"（textbook-sjp-zg-l18，"MTS社の菊池様"）——_split_kana_segments()
    # 靠非汉字组的字符去 hira 里搜索定位紧邻汉字组的读音，片假名有
    # `_kata_to_hira_char()` 转换成对应平假名再搜这条路，但"MTS"是拉丁字母，
    # 根本没有对应的假名字符可以拿去 hira="えむてぃーえすしゃ"里搜索——
    # 退化成兜底的"跳过N拍再搜"启发式，算出一堆不成形的乱码读音（真实表现：
    # "社"字被注成"ィーエスしゃ"）。拉丁字母缩写+汉字后缀这个组合是全新的
    # 边界情况，split_kana_segments 现有的两条锚点定位路径（送假名字面匹配/
    # 片假名转平假名匹配）都覆盖不到，直接给出人工核实过的拆分结果绕开——
    # "MTS"是外来缩写本身不需要单独注音，只有"社"字需要读しゃ。
    "MTS社": [("MTS", None), ("社", "しゃ")],
}
...
def _resolve_hira(orig, hira, prev_orig, next_char=None, prev2_orig=None):
    ...
    if orig == "入り" and prev_orig == "に":
        return "はいり"
    # "国名+の方"该读かた（尊敬语的"…国的人们"，比如"中国の方"＝"中国人们"），
    # pykakasi 默认给孤立的"方"字读ほう（"方向/方面"这个更常见的用法，比如
    # "こちらの方が良い"）。两个读音都是真实存在的正确用法，不能无条件覆盖
    # "の方"——区分点在于"の"前面是不是一个国家/地区名：这种场合几乎总是在说
    # "这个国家的人"，不是"这个国家的方向/方面"。真实案例（textbook-sjp-zg-l18，
    # "中国の方にはちょっと…""外国の方に人気のある商品""中国の方にも召し上が
    # っていただける"）：三处都读成了ほう，语境（"受欢迎""喜欢""吃"这类只能
    # 对人做的动作）明确要求是"人"这个意思，该读かた。目前只在真实遇到的
    # 国家名上覆盖，没有做成"任何名词+の方=かた"这种过度泛化的规则（"こちらの
    # 方""田舎の方"这类表示方向/地区的用法依然大量存在，不能被误伤）。
    if orig == "方" and prev_orig == "の" and prev2_orig in ("中国", "外国"):
        return "かた"
    # "交わす"（交换，比如"握手を交わす"＝互相握手）该读かわす，pykakasi 把
    # "交わ"这个 token 默认读まじわ（"交わる"＝まじわる，"互相接触/相交"这个
    # 更常见训读词干的读音，"交わす"和"交わる"共用同一个"交わ"前缀，pykakasi
    # 猜错成了后者）。两个读音都是真实存在的独立动词，不能无条件覆盖——用
    # next_char 区分：实测过 pykakasi 对"交わ"之后紧跟的第一个假名，"交わす"
    # 整个活用形族（终止形す/连用形・た形・て形し/未然形さ/可能形せ）分别是
    # す/し/さ/せ，"交わる"整个活用形族（终止形る/連用形り/た形・て形っ/未然形
    # ら/可能形れ）分别是る/り/っ/ら/れ，两族首假名完全不重叠，可以放心用这
    # 一个字符集合区分整个活用形族，不用像"後に"那样一种活用形态加一条。真实
    # 案例（N2词汇01"0015.握手"例句"握手を交わす。"）：音频TTS实际读的是正确
    # 的かわす，但furigana显示成"交（まじわ）す"，两者不一致，用户听出来才
    # 发现——TTS本身没读错，是furigana读音数据源（pykakasi）猜错了。
    if orig == "交わ" and next_char in "すしさせ":
        return "かわ"
...
def _split_kana_segments(orig, hira):
    ...
    `_TOKEN_SPLIT_OVERRIDES` 里登记过的 token 直接按人工核实过的拆分结果
    返回，完全不看传入的 hira——这类词本来就是"全汉字、没有送假名锚点"
    所以才会落进下面的退化分支，绕开退化分支才是目的，不能让它们先跑一遍
    锚点定位逻辑（反正也找不到锚点）。"""
    if orig in _TOKEN_SPLIT_OVERRIDES:
        return [
            {"text": t, "kana": k} if k else {"text": t}
            for t, k in _TOKEN_SPLIT_OVERRIDES[orig]
        ]
    if orig == hira:
...
def _group_by_mondai_question(sentences, questions):
    ...
    for mrec in by_mondai:
        for qrec in mrec["questions"]:
            ...
            # unit：N2语法/词汇页专属（build_n2_reference_page.py 从内容模块的
            # UNITS[]["label"] 传进来），标记这个词条/语法点属于第几单元，供
