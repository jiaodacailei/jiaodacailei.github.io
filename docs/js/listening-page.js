...
(function() {
  ...
  if (document.body.classList.contains("has-title-dictate")) {
    ...
    document.querySelectorAll(".question-block").forEach(function(block) {
      ...
      // 发音写，不是只能凭记忆猜。
      //
      // 跟中文提示放在同一行（不是独立成行），不是塞进 .dictate-row 里面——
      ...
      var playBtnHtml = wordAudioEl
        ? '<button type="button" class="dictate-play-word">▶ 播放发音</button>'
        : "";

      var ui = document.createElement("div");
      ui.className = "dictate-ui q-title-dictate";
      ui.innerHTML =
        '<div class="dictate-hint-row"><div class="dictate-hint"></div>' + playBtnHtml + '</div>' +
        '<div class="dictate-row">' +
