...
- `docs/private/n2-listening/2014-12/` — 同系列第十三套真题，31 个小题、5 个大题、
  ...
  牵连出新问题。
- `docs/private/n2-listening/2014-07/` — 同系列第十四套真题，31 个小题、5 个大题、
  334 句。踩到"問題2听岔"的一个更严重的变体：以往"問題2被听成問題に"只是**大题级**
  的播报标记漏检（`mondai_starts` 缺了問題2，但問題2 内部自己的"1番"~"6番"编号
  正常，靠数字重新计数的兜底能自己触发边界重置），这次連問題2 自己的1番都没能
  触发重置——`find_item_boundaries.py` 自动识别结果显示問題1只有4道正常小题，
  第5道小题时长长达644.6秒（同大题中位数111.1秒），把問題2 整整6道小题全部吞了
  进去，直到問題2自己的"6番"文本（数字6，大于問題1已经出现过的最大编号4）才
  重新触发识别，被误标成"問題1 6番"。根因：問題1原本已经数到4番，問題2重新从
  1番数起时，"1番"这个标签在問題1范围内其实是**重复出现**（問題1自己在最开头
  也有一个"1番"），编号重置检测的触发条件是"下一个识别到的编号严格大于上一个"，
  問題2的1番/2番/3番/4番/5番全都小于等于問題1已经数到的4，一律被判定成
  "不是新边界"跳过，只有問題2自己的6番（大于4）才被接受成新item——这跟以往
  "問題2听岔"案例的漏检机制完全不同，是编号重置启发式本身在"新大题编号恰好比
  上一大题已数到的编号小"这种场景下的固有局限，不是转写质量问题。发现方式：
  `find_item_boundaries.py` 打出的单题时长异常警告足够明显（644.6秒 vs 中位数
  111.1秒），没有静默漏检。修法完全复用已有工具：核对转写文本确认問題2真实播报
  位置（"問題に 問題にではまず"出现在533.76秒），用 `--insert-mondai "2:533.76"`
  直接补一个問題2的 mondai_start，问题解决，不需要改工具代码——`--insert-mondai`
  设计时就是奔着"整个問題N播报标记漏检"去的，这次只是漏检的诱因（编号重置逻辑
  被上一大题的编号"顶住"了）跟以往案例（数字被听成假名"に"）不一样，处理路径
  是同一条。同一份录音問題4 7番编号被听成裸数字"7"（老熟人坑，`--insert
  "問題4:7番:2066.0"` 解决），問題5 3番（質問1/質問2格式）选项文本又一次没有
  对应真实音频（老熟人坑，round-2 阶段留空 raw_ids，`refine_boundaries.py`/
  `validate_boundaries.py` 跑完后在 `enriched_final.json` 上补回8句，這次
  質問2到录音结尾只剩1.55秒空隙可分给4个选项，每句约0.39秒，比以往案例都紧张，
  但仍然是非零时长、不影响播放，`validate_boundaries.py` 沒有报零时长错误）。
  全程无 `ITEM_RE` 误判（没有"N番の学校"类内容语义触发）、无语速异常、无回退。
